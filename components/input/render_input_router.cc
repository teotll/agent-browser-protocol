...
blink::mojom::InputEventResultState RenderInputRouter::FilterInputEvent(
  ...
  // Exempt gesture events: they are generated by the browser from physical
  // input that already passed this filter (e.g., GestureScrollUpdate from
  // mouse wheel events). They never carry kFromDebugger.
  if (abp_enabled_ &&
