...
blink::mojom::InputEventResultState RenderInputRouter::FilterInputEvent(
  ...
  // Block real system input (unless --allow-system-inputs).
  // Only allow events marked with kFromDebugger (from CDP/ABP).
  // Exempt scroll gesture events only: GestureScrollBegin/Update/End are
  // generated by the browser from wheel events that already passed this
  // filter (e.g., ABP's ForwardWheelEvent with kFromDebugger). They never
  // carry kFromDebugger. Other gesture events (pinch, tap) from real
  // trackpad input are blocked.
  if (abp_enabled_ &&
      !allow_system_inputs_ &&
      !(event.GetModifiers() & WebInputEvent::kFromDebugger)) {
    bool is_scroll_gesture =
        event.GetType() == WebInputEvent::Type::kGestureScrollBegin ||
        event.GetType() == WebInputEvent::Type::kGestureScrollUpdate ||
        event.GetType() == WebInputEvent::Type::kGestureScrollEnd;
    if (!is_scroll_gesture) {
      // For wheel events, return kConsumed so MouseWheelEventQueue won't
      // generate gesture scroll events that bypass this filter.
