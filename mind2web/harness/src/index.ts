...
async function main() {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (config.resume) {
      const resultPath = join(config.outputDir, task.task_id, "result.json");
      try {
  ...
  const summary = {
    ...
    skipped: tasks.length - results.length,
    model: config.model,
  ...
  console.log("");
  console.log(
    `Summary: ${summary.completed} completed, ${summary.failed} failed, ${summary.skipped} skipped`,
  );
  if (results.length > 0) {
    console.log(
